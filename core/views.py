...
from core.tasks import send_verification_email, send_password_change_confirmation_email
from core.utils import GoogleOAuthHandler, get_or_create_google_user
...
class CreateUserView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        with transaction.atomic():
            ...
            verify_link = (
                self.request.build_absolute_uri(reverse("core:verify_email"))
                + f"?token={token.token}"
            )
...
class PasswordChangeView(CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        with transaction.atomic():
            ...
            confirm_link = (
                self.request.build_absolute_uri(reverse("core:confirm_password_change"))
                + f"?token={token.token}"
            )
...
@extend_schema_view(post=extend_schema(summary="Logout and blacklist refresh token"))
